...
load('/sc/arion/projects/roussp01a/liting/Pf_25/output/bip_finemap_final_link.RData') # get from gwas_2target_BIP_finemap.R
...
# bip
bip_final_link$internalPromoter <- promoter_anno[bip_final_link$TargetTranscript,]$internalPromoter
bip_final_link$tss_start <- promoter_anno[bip_final_link$TargetTranscript,]@ranges@start
bip_final_link$tss_chr <- bip_final_link$seqnames
bip_final_link$tss_end <- promoter_anno[bip_final_link$TargetTranscript,]@ranges@start +1 
bip_final_link$tss_strand <- as.data.frame(promoter_anno)[bip_final_link$TargetTranscript,'strand']


# pops score
bip_pops <- read.table('/sc/arion/projects/CommonMind/roussp01a/INGELHEIM/POPS/files/final_pops/added_ridge_bip2.preds', sep='\t', header = T, row.names =1 )
...
bip_final_link$quantile <- bip_pops[bip_final_link$gene,'quantile']

# prioritized gene/isoform

bip_target_supple <- unique(bip_final_link[,c('rsid','finemap_posterior_probability','gene','TargetGene_symbol','TargetTranscript','order','quantile','BR_Region')])
bip_target_supple <- rename(bip_target_supple,c("PoPS_quantile"="quantile", "TargetGene"='gene'))
bip_target_supple$PoPS_quantile <- round(bip_target_supple$PoPS_quantile, 3)
bip_target_supple$finemap_posterior_probability <- round(bip_target_supple$finemap_posterior_probability, 3)
write.table( unique(bip_target_supple), file='/sc/arion/projects/roussp01a/liting/Pf_25/data/supple_tables/prioritized_genes_bip.txt',quote = F,sep='\t',row.names = F)


# Prepare data for ucsc/bip genome brower 
#Candi <- subset(bip_final_link, TargetGene_symbol%in%c('SLC66A2','CALN1','MYT1L','PLCH2','TBL1XR1')) 
Candi <- subset(bip_final_link, rsid%in%c("rs61554907","rs10994397",'',''))
Candi <- subset(bip_final_link)#, rsid%in%c("rs61554907","rs10994397"))
Candi <- subset(bip_final_link, TargetGene_symbol%in%c('MED24','FURIN','WDR82'))

## candidate snp bed file
candi_snp <- unique(Candi[,c('seqnames.1','start.1','end.1','rsid','finemap_posterior_probability')])
candi_snp$start.1 <- candi_snp$start.1 - 1
candi_snpx <- cbind(candi_snp[,1:4], score=1000,strand='.',candi_snp[,c('start.1','end.1')],itemRgb='241,66,60')
write.table(candi_snpx, file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/candi_snp_bip.txt',sep='\t',quote = F,row.names = F, col.names = F)

write.table( candi_snp[,c('seqnames.1','start.1','end.1','finemap_posterior_probability')],  
             file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/candi_snp_pip_bip.txt',sep='\t',quote = F,row.names = F, col.names = F )

write.table(candi_snpx, file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/lead_snp.txt',sep='\t',quote = F,row.names = F, col.names = F)
...
write.table(unique(link_inter), file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/link_inter_bip.txt',sep='\t',quote = F,row.names = F, col.names = F)
...
write.table(promoter_isoform_tss, file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/promoter_isoform_tss_bip.txt',sep='\t',quote = F,row.names = F, col.names = F)


# GWAS 
library(data.table)
gwas_bip <- read.delim(gzfile('/sc/arion/projects/roussp01b/resources/databases/gwas/bip2/daner_bip_pgc3_nm.gz'),header = T,sep='\t')[c(1,2,3,11)]
colnames(gwas_bip) <- c('CHROM','ID','POS','PVAL')
gwas_bip$CHROM <- paste0('chr',gwas_bip$CHROM)
lead_hg19 <-  unique(subset(gwas_bip,ID%in%Candi$rsid)[,1:4])
gwas_bip_candi <- c()
for (i in 1:nrow(lead_hg19)){
  gwas_bip_candi <- rbind(gwas_bip_candi, subset(gwas_bip, CHROM==as.character(lead_hg19[i,1]) & 
...
gwas_19 = with(gwas_bip_candi, GRanges(CHROM, IRanges(start=as.numeric(POS), end=as.numeric(POS) ), PVAL=PVAL,ID=ID ))
gwas_pos_38 <- as.data.frame(liftOver(gwas_19,ch))[,c(3:5,8)]
gwas_pos_38$PVAL <-  -log10(gwas_pos_38$PVAL)
write.table(gwas_pos_38, file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/gwas_pos_38_bip.txt',sep='\t',quote = F,row.names = F, col.names = F)

## lead_snp bed file
#track name="Lead snp" visibility=2 color="255,0,0"
# index_snp <- unique(Candi$rsid)
...
# write.table(lead_snp, file='/hpc/users/songl05/PF_25BR/data/ucsc/bip/lead_snp.txt',sep='\t',quote = F,row.names = F, col.names = F)
...
# MOTIF break (SNP)
load('/sc/arion/projects/CommonMind/roussp01a/INGELHEIM/atacseq/test/neuron_norm_none_BIC_4_in_0.05_CPM_1_in_0.1/files/bip_finemap_.Rdata' )
motifBrk <- result
...
xxx <- subset(bip_final_link,trait_buddy_rs%in%names(mtf_footp) & BR_Region=='BasalGanglia' #& order!=1 
              &gene %in%rownames(bip_pops)[(bip_pops$quantile> 0.9)])
...
for(RS in unique(xxx$trait_buddy_rs)){
  # RS <- 'rs35709455'
  gene <- subset(xxx,trait_buddy_rs==RS &BR_Region=='BasalGanglia')
...
target_n5 <- c()
for (Br in unique(bip_final_link$BR_Region)){
  grs <- subset(bip_final_link, BR_Region==Br)
...
# MOTIF break (SNP)
load('/sc/arion/projects/CommonMind/roussp01a/INGELHEIM/atacseq/test/neuron_norm_none_BIC_4_in_0.05_CPM_1_in_0.1/files/BD_finemap_motifbreakR.Rdata' )
motifBrk <- result
...
xxx <- subset(bip_final_link,rsid%in%names(mtf_footp) & 
              gene %in%rownames(bip_pops)[(bip_pops$quantile> 0.9)])
...
ft_path <- '/sc/arion/projects/CommonMind/roussp01a/INGELHEIM/atacseq/step3_neuron_width500/files/tobias/bindetect/'
for(RS in unique(xxx$rsid)){
  # RS <- 'rs10994397'
  gene <- subset(xxx,rsid==RS)
  TF <- subset(mtf_footp,SNP_id==RS)$geneSymbol
  footPrt_n <- dir(ft_path,pattern = TF)
  footPrt <- read.table(paste0(ft_path,footPrt_n,'/',footPrt_n, "_overview.txt"),header = T)
  overlap11 <- findOverlaps( makeGRangesFromDataFrame(footPrt[,1:3]), 
                             makeGRangesFromDataFrame(gene[1,c('seqnames.1','start.1','end.1')]%>%rename(c('seqnames.1'='seqnames','start.1'='start','end.1'='end'))) )
  if( length(overlap11) >= 1){print(c(gene[,'TargetGene_symbol'],RS))}
  footPrt["4419",]
}

mtf_footp['rs10994397',]
mtf_footp['rs61554907',]
